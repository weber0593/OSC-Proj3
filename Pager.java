...
public class Pager	{
	...
	//Random Second Chance.  Picks a random frame, if not recently used, replace, else set bit and pick another random frame
	static void custom(ArrayList<MemoryAccess> queue, Frame[] frames, int pageSize)	{
		...
		for(MemoryAccess access : queue) {
			...
			if(frame_number == -1) {
				...
				int randomInt = rand.nextInt(maxFrames);
				index_to_replace = randomInt;
